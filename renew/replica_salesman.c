...
int main(void) {
    ...
    // ordering[i][ncity]=ncity
    double beta[nbeta];
    // location of the cities on 2d plane. x[i][0]=x[i][ncity].
    double x[2][ncity + 1];
    int naccept[nbeta];
    ...
    for (int iter = 1; iter < niter + 1; iter++) {
        ...
        for (int ibeta = 0; ibeta < nbeta - 1; ibeta++) {
            double action_init = calc_distance(x, ordering, ibeta) * beta[ibeta] +
                                 calc_distance(x, ordering, ibeta + 1) * beta[ibeta + 1];
            double action_fin = calc_distance(x, ordering, ibeta) * beta[ibeta + 1] +
                                calc_distance(x, ordering, ibeta + 1) * beta[ibeta];
            /*******************/
            /* Metropolis test */
            /*******************/
            double metropolis = (double)rand() / RAND_MAX;
    ...
    for (int icity = 0; icity < ncity + 1; icity++) {
        ...
        fprintf(outputfile, "%lf     %lf\n", x[0][ordering[nbeta - 1][icity]],
                x[1][ordering[nbeta - 1][icity]]);
    }
    fclose(outputfile);
