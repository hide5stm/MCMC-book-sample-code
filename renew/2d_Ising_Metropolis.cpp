...
/*********************************/
double calc_action(const int spin[nx][ny], const double coupling_J, const double coupling_h,
...
/***********************************************/
double calc_action_change(const int spin[nx][ny], const double coupling_J, const double coupling_h,
                          const double temperature, const int ix, const int iy) {
    ...
    int sum2_change = 2 * spin[ix][iy] * spin[ixp1][iy] + 2 * spin[ix][iy] * spin[ix][iyp1] +
                      2 * spin[ix][iy] * spin[ixm1][iy] + 2 * spin[ix][iy] * spin[ix][iym1];
...
int main() {
    ...
    for (long int iter = 0; iter != niter; iter++) {
        ...
        /*******************/
        if ((iter + 1) % nskip == 0) {
            std::cout << std::fixed << std::setprecision(4) << total_spin << "   " << energy << "   "
                      << (double)naccept / (iter + 1) << std::endl;
            outputfile << std::fixed << std::setprecision(4) << total_spin << "   " << energy << "   "
                       << (double)naccept / (iter + 1) << std::endl;
        }
    }
    outputfile.close();
