...
/********************************/
int calc_delh(std::complex<double> (&delh)[nmat][nmat], const std::complex<double> phi[nmat][nmat]) {
...
/*** Molecular evolution ***/
/***************************/
int Molecular_Dynamics(std::complex<double> (&phi)[nmat][nmat], double &ham_init, double &ham_fin) {
...
int main() {
    ...
    for (int iter = 0; iter != niter; iter++) {
        ...
        std::cout << std::fixed << std::setprecision(6) << sum_action / ((iter + 1) * nmat * nmat) << "   "
                  << ((double)naccept) / (iter + 1) << std::endl;

        outputfile << std::fixed << std::setprecision(6) << sum_action / ((iter + 1) * nmat * nmat) << "   "
                   << ((double)naccept) / (iter + 1) << std::endl;
    }
    outputfile.close();
