...
/*********************************************/
double heat_bath_probability(const int spin[nx][ny], const double coupling_J, const double coupling_h,
...
int main() {
    ...
    for (long int iter = 0; iter != niter; iter++) {
        ...
        /*******************/
        if ((iter + 1) % nskip == 0) {
            std::cout << std::fixed << std::setprecision(4) << total_spin << "   " << energy << std::endl;
            outputfile << std::fixed << std::setprecision(4) << total_spin << "   " << energy << std::endl;
        }
    }
    outputfile.close();
    /*************************/
