...
int calc_delh(const double x[ndim], const double A[ndim][ndim], double (&delh)[ndim]) {
    for (int idim = 0; idim != ndim; idim++) {
        delh[idim] = 0e0;
    }
...
/*** Molecular evolution ***/
/***************************/
int Molecular_Dynamics(double (&x)[ndim], const double A[ndim][ndim], double &ham_init, double &ham_fin) {
    double p[ndim];
    double delh[ndim];
    double r1, r2;
...
int main() {
    ...
    for (int iter = 0; iter != niter; iter++) {
        ...
        /*******************/
        if ((iter + 1) % 10 == 0) {
            std::cout << std::fixed << std::setprecision(6) << x[0] << "   " << x[1] << "   " << x[2] << "   "
                      << ((double)naccept) / ((double)iter + 1) << std::endl;

            outputfile << std::fixed << std::setprecision(6) << x[0] << "   " << x[1] << "   " << x[2]
                       << "   " << ((double)naccept) / ((double)iter + 1) << std::endl;
        }
    }
    outputfile.close();
