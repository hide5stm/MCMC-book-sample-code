...
double calc_action(const double A[ndim][ndim], const double mu[ndim]) {
    ...
    action = action + 0.5e0 * A[0][0] * A[0][0] + 0.5e0 * A[1][1] * A[1][1] + A[0][1] * A[0][1] +
             0.5e0 * mu[0] * mu[0] + 0.5e0 * mu[1] * mu[1];
...
/**************************************/
double calc_hamiltonian(const double p_A[ndim][ndim], const double p_mu[ndim], const double A[ndim][ndim],
                        const double mu[ndim]) {
...
int calc_delh(const double A[ndim][ndim], const double mu[ndim], double (&delh_A)[ndim][ndim],
              double (&delh_mu)[ndim]) {
    ...
    delh_A[0][1] =
        A[0][1] + (mu[0] * mu[1] - av_x * mu[1] - av_y * mu[0] + av_xy - A_inv[0][1]) * 0.5e0 * nsample;
...
/*** Molecular evolution ***/
/***************************/
int Molecular_Dynamics(double (&A)[ndim][ndim], double (&mu)[ndim], double &ham_init, double &ham_fin) {
...
int main() {
    ...
    for (int iter = 0; iter != niter; iter++) {
        ...
        // output
        if ((iter + 1) % nskip == 0) {
            std::cout << std::fixed << std::setprecision(6) << A[0][0] << "   " << A[1][1] << "   " << A[0][1]
                      << "   " << mu[0] << "   " << mu[1] << "   " << (double)naccept / (iter + 1)
                      << std::endl;
        }
    }
    return 0;
